...
```bash
touch -p /src/components/socialWidget.jsx
```
...
It's okay! What's happening is actually pretty simple, it's just weird if you haven't seen `array.map()`or an anonymous callback function before. Let's dive in and see what's going on. 
